...
const props = defineProps(['size', 'idPost', 'idCom', 'colors', 'message', 'redirect'])
const show = ref(false)

const handleDelete = () => {
    if (props.idCom) {
        comments.deletecomment(props.idCom)
        comments.removecomments(props.idCom)
    } else {
        post.deletepost(props.idPost)
        post.removepostes(props.idPost)
...
<template>
    ...
    <div>
        <p :class="`flex  items-center cursor-pointer gap-x-1 ${props.colors}`" data-modal-target="popup-modal"
            @click="show = true">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" :width="`${props.size}`"
                :height="`${props.size}`">
                <path
            ...
            <span v-if="props.message" class="lowercase text-sm bg-none tracking-normal"> Delete </span>
            <span v-else></span>
        </p>
    ...
    <div v-if="show" @mousedown="handledMouse" class="rounded-lg p-4 gap-y-6 bg-gray-100">
        ...
        <div class="fixed inset-0 z-40 min-h-full overflow-y-auto overflow-x-hidden transition flex items-center">
            ...
            <!-- Modal -->
            <div class="modal relative w-full cursor-pointer pointer-events-none transition my-auto p-4">
                <div
